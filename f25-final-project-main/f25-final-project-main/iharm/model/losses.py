    ...
    def forward(self, pred, label):
        if torch.is_tensor(pred):
            ...
            for pred_img, label_img in zip(pred, label):
                if pred_img.device != label_img.device:
                    label_img = label_img.to(pred_img.device)
                mse = torch.mean((pred_img - label_img) ** 2)
                loss.append(mse)
            loss = torch.stack(loss)
        return loss
...
class ColorDistance(Loss):
    def __init__(self, pred_name='color_dis'):
        super(ColorDistance, self).__init__(pred_outputs=(pred_name,), gt_outputs=())

    def forward(self, color_dis):  # [1, 3, 8, H, W]
        product = torch.prod(color_dis, dim=2)
        loss = torch.sum(product)
        return loss

class CoordDistance(Loss):
    def __init__(self, pred_name='coord_dis'):
        super(CoordDistance, self).__init__(pred_outputs=(pred_name,), gt_outputs=())

    def forward(self, coord_dis):  # [1, 3, 2, H, W]
        product_w = torch.prod(coord_dis[:, :, [0], :, :], dim=4)
        product_h = torch.prod(coord_dis[:, :, [1], :, :], dim=3)

        loss = torch.sum(product_w) + torch.sum(product_h)
        return loss


class Sparse(Loss):
    def __init__(self, pred_name='lut3d_weights'):
        super(Sparse, self).__init__(pred_outputs=(pred_name,), gt_outputs=())

    def forward(self, weights):  # [1, 3, 2, H, W]

        loss = torch.mean(weights.pow(2))
        return loss


class Smooth(Loss):
    def __init__(self, pred_name='basis_luts'):
        super(Smooth, self).__init__(pred_outputs=(pred_name,), gt_outputs=())

    def forward(self, basis_luts):  # [1, 3, 2, H, W]
...
class Monotonicity(Loss):
    def __init__(self, pred_name='basis_luts'):
        super(Monotonicity, self).__init__(pred_outputs=(pred_name,), gt_outputs=())
