...
class Predictor(object):
    ...
    def predict(self, image, image_highres, mask, mask_highres, return_numpy=True):
        ...
        with torch.no_grad():
            ...
            if 'class_weight' in output:
                class_weight = output['class_weight'].squeeze()

            color_dis = None
            if 'color_dis' in output:
                color_dis = output['color_dis']
        ...
        if return_numpy:
            return predicted_image.cpu().numpy(), output_fullres.cpu().numpy(), color_dis.cpu().numpy()
        else:
            return predicted_image, output_fullres, color_dis
