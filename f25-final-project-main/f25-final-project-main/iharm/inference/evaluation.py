...
def evaluate_dataset(dataset, predictor, metrics_hub_lowres, metrics_hub_fullres, visdir=None, csv_dir=None):
    ...
    for sample_i in trange(len(dataset), desc=f'Testing on {metrics_hub_lowres.name}'):
        ...
        # if not (raw_image.shape[0] > 5500 or raw_image.shape[1] > 5500):
        #     continue
        ...
        image_highres = sample_highres['image']
        sample_mask_highres = sample_highres['object_mask']

        predict_start = time()
        pred, pred_fullres, color_dis = predictor.predict(image_lowres, raw_image, sample_mask, sample_mask_highres, return_numpy=False)
        ...
        # #pred = torch.as_tensor(image_lowres, dtype=torch.float32).to(predictor.device)
        # #pred_fullres = torch.as_tensor(image_highres, dtype=torch.float32).to(predictor.device)
        ...
        lpips = fullres_result[4]
        ssim = fullres_result[5]
        height, width = raw_mask.shape
        mask_area = raw_mask.sum().item()
        metric_data.append([imname, mse, psnr, fmse, lpips, ssim, height, width, mask_area])
        ...
        if visdir and sample_i % 1 == 0:
            ...
            harm = pred_fullres[:, :, ::-1]
            real = bdata['target_image'][:, :, ::-1]
            mask = raw_mask
            comp = raw_image.cpu().numpy()[:, :, ::-1]

            cv2.imwrite(os.path.join(visdir, f'{imname}_harmonized.jpg'), harm, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
            cv2.imwrite(os.path.join(visdir, f'{imname}_real.jpg'), real, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
            cv2.imwrite(os.path.join(visdir, f'{imname}_mask.jpg'), mask, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
            cv2.imwrite(os.path.join(visdir, f'{imname}_comp.jpg'), comp, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
    ...
    df = pd.DataFrame(metric_data, columns=['Name', 'MSE', 'PSNR', 'fMSE', 'LPIPS', 'SSIM', 'height', 'width', 'mask_area'])
